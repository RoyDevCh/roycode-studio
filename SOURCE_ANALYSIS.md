...
| React/Ink terminal UI | Yes | Partially | The original snapshot uses a richer Ink app shell; RoyCode now has an Ink-style TUI launcher with workspace/session panels, shortcut hints, recent input history, and a live output pane over its local CLI core, but it is still simpler than the original REPL app. |
