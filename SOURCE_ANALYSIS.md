...
| React/Ink terminal UI | Yes | Partially | The original snapshot uses a richer Ink app shell; RoyCode now has an Ink-style TUI launcher over its local CLI core, but it is still simpler than the original REPL app. |
...
| LSP / code intelligence | Yes | Partially | RoyCode now has a local TypeScript/JavaScript LSP subset for diagnostics, definitions, implementations, references, rename preview, hover, document symbols, and workspace symbols; broader language-server coverage is still absent. |
