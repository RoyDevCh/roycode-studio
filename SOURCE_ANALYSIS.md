...
| React/Ink terminal UI | Yes | Partially | The original snapshot uses a richer Ink app shell; RoyCode uses a lighter readline-style CLI. |
...
| Multi-agent task orchestration | Yes | Partially | RoyCode now has project/user `.claude/agents`, `run_subagent`, background tasks, and agent task tools, but not the snapshot's full teammate/subagent platform. |
| Team runtime | Yes | Partially | RoyCode now has local teams, member management, and team-run workflows, but not the snapshot's full teammate platform or remote orchestration layer. |
| Worktree workflow | Yes | Partially | RoyCode now supports local git worktree listing, creation, removal, and teleport-style workspace switching, but not the snapshot's full managed isolation model. |
...
| LSP / code intelligence | Yes | Partially | RoyCode now has a local TypeScript/JavaScript LSP subset for diagnostics, definitions, references, hover, and symbols; broader language-server coverage is still absent. |
